...
class MultiDownload(DownloaderCore):
    ...
    def spawn_thread_(self, fh, i, chunks_tuple):
        """"""
        #Requested Range Not Satisfiable, bug.
        if chunks_tuple[1] is None and chunks_tuple[0] > 0:
            chunks_tuple = (chunks_tuple[0] - 1, None)
        th = threading.Thread(group=None, target=self.thread_download_, name=None, args=(fh, i, chunks_tuple))
        th.start()
        return th

    def threaded_download_manager_(self, fh):
        """"""
        ...
        th_list = [self.spawn_thread_(fh, i, chunks_tuple) for i, chunks_tuple in enumerate(self.chunks[:])
    ...
    def thread_download_(self, fh, i, chunk_range):
        print "beta"
        complete = 0
        running_flag = False
        while True:
